...
pub const Renderer = struct {
   ...
   _vulkan_command_pool_transfer       : VulkanCommandPool,
   ...
   _vulkan_descriptor_sets             : VulkanDescriptorSetList,
   ...
   pub const CreateError = error {
      ...
      VulkanDescriptorPoolCreateFailure,
      VulkanCommandBuffersGraphicsCreateFailure,
      VulkanCommandBuffersTransferCreateFailure,
      VulkanDescriptorSetsCreateFailure,
      VulkanSemaphoresImageAvailableCreateFailure,
      VulkanSemaphoresRenderFinishedCreateFailure,
      VulkanFencesInFlightCreateFailure,
   };

   const VULKAN_COMMAND_BUFFERS_TRANSFER_COUNT     = 1;
   const VULKAN_COMMAND_BUFFERS_TRANSFER_PRIMARY   = 0;
   ...
   pub fn create(window : * const f_present.Window, allocator : std.mem.Allocator, create_options : CreateOptions) CreateError!@This() {
      ...
      const vulkan_command_pool_transfer = VulkanCommandPool.create(
         vulkan_device.vk_device, vulkan_physical_device.queue_family_indices.transfer,
      ) catch return error.VulkanCommandPoolTransferCreateFailure;
      errdefer vulkan_command_pool_transfer.destroy(vulkan_device.vk_device);

      const vulkan_descriptor_pool = VulkanDescriptorPool.create(
         vulkan_device.vk_device, create_options.frames_in_flight,
      ) catch return error.VulkanDescriptorPoolCreateFailure;
      errdefer vulkan_descriptor_pool.destroy(vulkan_device.vk_device);
      ...
      const vulkan_command_buffers_transfer = VulkanCommandBufferList.create(
         allocator,
         VULKAN_COMMAND_BUFFERS_TRANSFER_COUNT,
         vulkan_device.vk_device,
         vulkan_command_pool_transfer.vk_command_pool,
      ) catch return error.VulkanCommandBuffersTransferCreateFailure;
      errdefer vulkan_command_buffers_transfer.destroy(allocator);
      ...
      const vulkan_index_buffer = VulkanIndexBuffer.create(
         vulkan_device.vk_device,
         vulkan_command_buffers_transfer.vk_command_buffers[VULKAN_COMMAND_BUFFERS_TRANSFER_PRIMARY],
         &vulkan_physical_device,
         &vulkan_device.queues,
         create_options.render_mesh.indices,
      ...
      const vulkan_descriptor_sets = VulkanDescriptorSetList.create(
         allocator,
         create_options.frames_in_flight,
         vulkan_device.vk_device,
         vulkan_descriptor_pool.vk_descriptor_pool,
         vulkan_graphics_pipeline.vk_descriptor_set_layout,
         vulkan_uniform_buffers.vulkan_uniform_buffers,
      ) catch return error.VulkanDescriptorSetsCreateFailure;
      errdefer vulkan_descriptor_sets.destroy(allocator);

      const vulkan_semaphores_image_available = VulkanSemaphoreList.create(
         allocator,
         create_options.frames_in_flight,
         vulkan_device.vk_device,
      ) catch return error.VulkanSemaphoresImageAvailableCreateFailure;
      errdefer vulkan_semaphores_image_available.destroy(allocator, vulkan_device.vk_device);

      const vulkan_semaphores_render_finished = VulkanSemaphoreList.create(
         allocator,
         create_options.frames_in_flight,
         vulkan_device.vk_device,
      ) catch return error.VulkanSemaphoresRenderFinishedCreateFailure;
      errdefer vulkan_semaphores_render_finished.destroy(allocator, vulkan_device.vk_device);

      const vulkan_fences_in_flight = VulkanFenceList.create(
         allocator,
         create_options.frames_in_flight,
         vulkan_device.vk_device,
      ...
      return @This(){
         ...
         ._render_mesh                       = create_options.render_mesh,
         ...
         ._vulkan_command_pool_transfer      = vulkan_command_pool_transfer,
         ._vulkan_descriptor_pool            = vulkan_descriptor_pool,
   ...
   pub fn destroy(self : @This()) void {
      ...
      self._vulkan_descriptor_sets.destroy(allocator);
      self._vulkan_command_buffers_transfer.destroy(allocator);
      self._vulkan_command_buffers_graphics.destroy(allocator);
      self._vulkan_descriptor_pool.destroy(vk_device);
      self._vulkan_command_pool_transfer.destroy(vk_device);
   ...
   pub fn renderFrame(self : * @This()) RenderError!void {
      while (try self._renderFrameWithSwapchainUpdates() == false) {}
      return;
   }

   fn _renderFrameWithSwapchainUpdates(self : * @This()) RenderError!bool {
      ...
      switch (vk_result) {
         ...
         c.VK_TIMEOUT                                    => {},
         c.VK_NOT_READY                                  => {},
         c.VK_SUBOPTIMAL_KHR                             => recreate_swapchain_at_end = true,
         c.VK_ERROR_OUT_OF_HOST_MEMORY                   => return error.OutOfMemory,
         c.VK_ERROR_OUT_OF_DEVICE_MEMORY                 => return error.OutOfMemory,
         c.VK_ERROR_DEVICE_LOST                          => return error.DeviceLost,
      ...
      try _recordRenderPass(self, vk_command_buffer_graphics, vk_descriptor_set, vk_framebuffer, self._clear_color);
      ...
      switch (vk_result) {
         ...
         c.VK_ERROR_OUT_OF_DEVICE_MEMORY                 => return error.OutOfMemory,
         c.VK_ERROR_DEVICE_LOST                          => return error.DeviceLost,
         c.VK_ERROR_OUT_OF_DATE_KHR                      => recreate_swapchain_at_end = true,
         c.VK_ERROR_SURFACE_LOST_KHR                     => return error.SurfaceLost,
         c.VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT  => return error.UnknownError,
         else                                            => unreachable,
      }
      ...
      if (recreate_swapchain_at_end == true) {
         self._recreateSwapchain() catch return error.SwapchainRecreateFailure;
      }
   ...
   fn _recordRenderPass(self : * const @This(), vk_command_buffer : c.VkCommandBuffer, vk_descriptor_set : c.VkDescriptorSet, vk_framebuffer : c.VkFramebuffer, clear_color : ClearColor) RenderError!void {
      var vk_result : c.VkResult = undefined;
      ...
      c.vkCmdBindPipeline(vk_command_buffer, c.VK_PIPELINE_BIND_POINT_GRAPHICS, self._vulkan_graphics_pipeline.vk_pipeline);

      const VERTEX_BUFFER_COUNT : u32 = 1;

      const vk_vertex_buffers = [VERTEX_BUFFER_COUNT] c.VkBuffer {
         self._vulkan_vertex_buffer.vulkan_buffer.vk_buffer,
      };

      const vk_vertex_buffer_offsets = [VERTEX_BUFFER_COUNT] c.VkDeviceSize {
         0,
      };

      c.vkCmdBindVertexBuffers(vk_command_buffer, 0, VERTEX_BUFFER_COUNT, &vk_vertex_buffers, &vk_vertex_buffer_offsets);

      c.vkCmdBindIndexBuffer(vk_command_buffer, self._vulkan_index_buffer.vulkan_buffer.vk_buffer, 0, c.VK_INDEX_TYPE_UINT32);

      const vk_viewport = c.VkViewport{
         .x          = 0.0,
         .y          = 0.0,
         .width      = @floatFromInt(self._vulkan_swapchain_configuration.extent.width),
         .height     = @floatFromInt(self._vulkan_swapchain_configuration.extent.height),
         .minDepth   = 0.0,
         .maxDepth   = 1.0,
      };
      ...
      c.vkCmdBindDescriptorSets(vk_command_buffer, c.VK_PIPELINE_BIND_POINT_GRAPHICS, self._vulkan_graphics_pipeline.vk_pipeline_layout, 0, 1, &vk_descriptor_set, 0, null);

      c.vkCmdDrawIndexed(vk_command_buffer, @intCast(self._render_mesh.indices.len), 1, 0, 0, 0);
   ...
   const RecreateSwapchainError = error {
   ...
   fn _recreateSwapchain(self : * @This()) RecreateSwapchainError!void {
      var vk_result : c.VkResult = undefined;
...
const VulkanCommandBufferList = struct {
   ...
   pub fn create(allocator : std.mem.Allocator, count : u32, vk_device : c.VkDevice, vk_command_pool : c.VkCommandPool) CreateError!@This() {
      var vk_result : c.VkResult = undefined;
      ...
      var vk_command_buffers = try allocator.alloc(c.VkCommandBuffer, @as(usize, count));
      errdefer allocator.free(vk_command_buffers);

      vk_result = c.vkAllocateCommandBuffers(vk_device, &vk_info_allocate_command_buffer, vk_command_buffers.ptr);
      switch (vk_result) {
         c.VK_SUCCESS                     => {},
         c.VK_ERROR_OUT_OF_HOST_MEMORY    => return error.OutOfMemory,
         c.VK_ERROR_OUT_OF_DEVICE_MEMORY  => return error.OutOfMemory,
   ...
   pub fn destroy(self : @This(), allocator : std.mem.Allocator) void {
      allocator.free(self.vk_command_buffers);
      return;
   }
...
const VulkanSemaphoreList = struct {
   vk_semaphores  : [] c.VkSemaphore,

   pub const CreateError = error {
      OutOfMemory,
   };

   pub fn create(allocator : std.mem.Allocator, count : u32, vk_device : c.VkDevice) CreateError!@This() {
      var vk_semaphores = try allocator.alloc(c.VkSemaphore, @as(usize, count));
      errdefer allocator.free(vk_semaphores);

      for (vk_semaphores, 0..count) |*vk_semaphore_dest, i| {
         errdefer for (vk_semaphores[0..i]) |vk_semaphore_free| {
            c.vkDestroySemaphore(vk_device, vk_semaphore_free, null);
         };
      ...
      errdefer for (vk_semaphores) |vk_semaphore_free| {
         c.vkDestroySemaphore(vk_device, vk_semaphore_free, null);
      };
   ...
   pub fn destroy(self : @This(), allocator : std.mem.Allocator, vk_device : c.VkDevice) void {
      ...
      allocator.free(self.vk_semaphores);
      return;
   }
};

const VulkanFenceList = struct {
   vk_fences : [] c.VkFence,
   ...
   pub fn create(allocator : std.mem.Allocator, count : u32, vk_device : c.VkDevice) CreateError!@This() {
      var vk_fences = try allocator.alloc(c.VkFence, @as(usize, count));
      errdefer allocator.free(vk_fences);

      for (vk_fences, 0..count) |*vk_fence_dest, i| {
         errdefer for (vk_fences[0..i]) |vk_fence_free| {
            c.vkDestroyFence(vk_device, vk_fence_free, null);
         };
      ...
      errdefer for (vk_fences) |vk_fence_free| {
         c.vkDestroyFence(vk_device, vk_fence_free, null);
      };
...
const VulkanBuffer = struct {
   ...
   pub fn create(vk_device : c.VkDevice, vulkan_physical_device : * const VulkanPhysicalDevice, create_options : * const CreateOptions) CreateError!@This() {
      ...
      var vk_buffer_memory_requirements : c.VkMemoryRequirements = undefined;
      c.vkGetBufferMemoryRequirements(vk_device, vk_buffer, &vk_buffer_memory_requirements);
      ...
      const vk_info_allocate_memory = c.VkMemoryAllocateInfo{
         .sType            = c.VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
         .pNext            = null,
         .allocationSize   = vk_buffer_memory_requirements.size,
         .memoryTypeIndex  = vk_memory_type_index,
      };

      var vk_device_memory : c.VkDeviceMemory = undefined;
      vk_result = c.vkAllocateMemory(vk_device, &vk_info_allocate_memory, null, &vk_device_memory);
      switch (vk_result) {
         c.VK_SUCCESS                                    => {},
         c.VK_ERROR_OUT_OF_HOST_MEMORY                   => return error.OutOfMemory,
         c.VK_ERROR_OUT_OF_DEVICE_MEMORY                 => return error.OutOfMemory,
   ...
   pub fn copyToDevice(src : c.VkBuffer, dst : c.VkBuffer, size : c.VkDeviceSize, vk_command_buffer : c.VkCommandBuffer, vk_queue : c.VkQueue) CreateError!void {
      var vk_result : c.VkResult = undefined;
...
const VulkanVertexBuffer = struct {
   ...
   pub fn create(vk_device : c.VkDevice, vk_command_buffer_transfer : c.VkCommandBuffer, vulkan_physical_device : * const VulkanPhysicalDevice, vulkan_queues : * const VulkanDevice.Queues, vertices : [] const Renderer.Vertex) CreateError!@This() {
      ...
      try VulkanBuffer.copyToDevice(vulkan_buffer_staging.vk_buffer, vulkan_buffer_vertex.vk_buffer, buffer_size_bytes, vk_command_buffer_transfer, vulkan_queues.transfer);
...
const VulkanIndexBuffer = struct {
   vulkan_buffer  : VulkanBuffer,
   ...
   pub fn create(vk_device : c.VkDevice, vk_command_buffer_transfer : c.VkCommandBuffer, vulkan_physical_device : * const VulkanPhysicalDevice, vulkan_queues : * const VulkanDevice.Queues, indices : [] const Renderer.IndexBufferType) CreateError!@This() {
      var vk_result : c.VkResult = undefined;
      ...
      const vk_memory_mapping_vertices : [*] Renderer.IndexBufferType = @ptrCast(@alignCast(vk_memory_mapping orelse unreachable));
      @memcpy(vk_memory_mapping_vertices, indices);

      try VulkanBuffer.copyToDevice(vulkan_buffer_staging.vk_buffer, vulkan_buffer_index.vk_buffer, buffer_size_bytes, vk_command_buffer_transfer, vulkan_queues.transfer);
...
const VulkanDescriptorPool = struct {
   ...
   pub fn create(vk_device : c.VkDevice, maximum_allocations : u32) CreateError!@This() {
      var vk_result : c.VkResult = undefined;

      const vk_descriptor_pool_size = c.VkDescriptorPoolSize{
         .type             = c.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
         .descriptorCount  = maximum_allocations,
      };
      ...
      const vk_info_create_descriptor_pool = c.VkDescriptorPoolCreateInfo{
         ...
         .pNext         = null,
         .flags         = 0x00000000,
         .maxSets       = maximum_allocations,
         .poolSizeCount = 1,
         .pPoolSizes    = &vk_descriptor_pool_size,
      };
...
const VulkanDescriptorSetList = struct {
   ...
   pub fn create(allocator : std.mem.Allocator, count : u32, vk_device : c.VkDevice, vk_descriptor_pool : c.VkDescriptorPool, vk_descriptor_set_layout : c.VkDescriptorSetLayout, vk_uniform_buffers : [] VulkanUniformBuffer) CreateError!@This() {
      var vk_result : c.VkResult = undefined;

      var vk_descriptor_set_layouts = try allocator.alloc(c.VkDescriptorSetLayout, count);
      defer allocator.free(vk_descriptor_set_layouts);

      var vk_descriptor_sets = try allocator.alloc(c.VkDescriptorSet, count);
      errdefer allocator.free(vk_descriptor_sets);

      for (vk_descriptor_set_layouts) |*vk_descriptor_set_layout_dest| {
         vk_descriptor_set_layout_dest.* = vk_descriptor_set_layout;
      }

      const vk_info_descriptor_set_allocate = c.VkDescriptorSetAllocateInfo{
         .sType               = c.VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
         .pNext               = null,
         .descriptorPool      = vk_descriptor_pool,
         .descriptorSetCount  = @intCast(vk_descriptor_set_layouts.len),
         .pSetLayouts         = vk_descriptor_set_layouts.ptr,
      };

      vk_result = c.vkAllocateDescriptorSets(vk_device, &vk_info_descriptor_set_allocate, vk_descriptor_sets.ptr);
      switch (vk_result) {
         c.VK_SUCCESS => {},
         c.VK_ERROR_OUT_OF_HOST_MEMORY    => return error.OutOfMemory,
         c.VK_ERROR_OUT_OF_DEVICE_MEMORY  => return error.OutOfMemory,
      ...
      for (vk_descriptor_sets, vk_uniform_buffers[0..count]) |vk_descriptor_set, vk_uniform_buffer| {
         const vk_info_descriptor_buffer = c.VkDescriptorBufferInfo{
            .buffer  = vk_uniform_buffer.vulkan_uniform_buffer.vk_buffer,
            .offset  = 0,
            .range   = @intCast(@sizeOf(Renderer.UniformBufferObject)),
         };
         ...
         const vk_write_descriptor_set = c.VkWriteDescriptorSet{
            ...
            .descriptorType   = c.VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
            .pImageInfo       = null,
            .pBufferInfo      = &vk_info_descriptor_buffer,
            .pTexelBufferView = null,
         };

         c.vkUpdateDescriptorSets(vk_device, 1, &vk_write_descriptor_set, 0, null);
      }
   ...
   pub fn destroy(self : @This(), allocator : std.mem.Allocator) void {
      allocator.free(self.vk_descriptor_sets);
      return;
   }
};
